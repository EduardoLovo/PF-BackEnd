...
const getTarefa = async (req, res) => {
  await tarefaModel.find({}, (err, tarefa) => {
    if (err) {
      return res.status(400).send(`Tarefa invalida ${err}`);
    }
    if (!tarefa) {
...
const createTarefa = (req, res) => {
  ...
  tarefa
    .save()
    .then(() => res.status(201).send(`Tarefa ${body.title}  criada com sucesso`))
    .catch(() =>
...
const deleteTarefa = async (req, res) => {
  await tarefaModel
    ...
    .catch((err) => res.status(404).send(`Tarefa não encontrada: ${err.message}`));
};
